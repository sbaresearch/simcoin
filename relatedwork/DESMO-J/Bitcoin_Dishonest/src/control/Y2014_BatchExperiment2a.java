...
public class Y2014_BatchExperiment2a {
	...
	public static void main(String[] args) {
		...
		try {
			writer = new BufferedWriter(new OutputStreamWriter(
					new FileOutputStream("batch_out2a.csv")));
			...
			String o =    "delay, m0, ms0, l0, ml0, r0, dt0, dtc0, dts0, " + 
					             "m1, ms1, l1, ml1, r1, dt1, dtc1, dts1, " +
				                 "m2, ms2, l2, ml2, r2, dt2, dtc2, dts2, " +
				               	 "m3, ms3, l3, ml3, r3, dt3, dtc3, dts3, " +
				                 "time(min)\r\n";
			System.out.print(o);
			...
			//for (int delay_set = 58; delay_set >= 0; delay_set--) {
				...
				for (int e = 1; e <= 4; e++) {
					...
					double lost = get(result,"m")-get(result,"ma");

					out +=  threeAfterDecPoint.format(get(result,"m")).replace(',','.') + ", " + 	
							threeAfterDecPoint.format(get(result,"ma")).replace(',','.') + ", " + 
							threeAfterDecPoint.format(lost).replace(',','.') + ", " + 
							(lost < 0.00001 ? " " : threeAfterDecPoint.format(get(result,"ma") / lost).replace(',','.'))+ ", " + 
