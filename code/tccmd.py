...
def create(node, zones, latency):
    ...
    for index, zone_tuple in enumerate(sorted_zones):
        ...
        cmds.append('tc filter add dev eth0 parent 1: protocol ip prio {} u32'
                    ' match ip dst {} flowid 1:{}'.format(index + 1, zone.network, index + 2))
    ...
    for index, zone_tuple in enumerate(sorted_zones):
        ...
        cmds.append('tc qdisc add dev eth0 parent 1:{} handle {}: netem delay {}ms'.format(
            index + 2, (index + 2) * 10, aggregated_latency
        ))
