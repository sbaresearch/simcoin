...
xyplot(memory$usage ~ memory$timestamp - sim_start, xlab = 'Time [s]', ylab = 'Memory usage [%]', t = 'b' )
```
...
cpu_time_delta           <- data.frame(cpu_time$timestamp[-1], diff(cpu_time$total), diff(cpu_time$idle))
colnames(cpu_time_delta) <- c('timestamp', 'total', 'idle')
cpu_time_delta$usage     <- (1 - cpu_time_delta$idle / (cpu_time_delta$total + cpu_time_delta$idle)) * 100

xyplot(cpu_time_delta$usage ~ cpu_time_delta$timestamp  - sim_start, xlab = 'Time [s]', ylab = 'CPU usage [%]', t = 'b' )
```
...
bwplot(tick_infos$duration, ylab='Tick duration [s]')
```

```{r tick_duration_xyplot, fig.cap='Tick duration over time', fig.env='figure*'}
xyplot(tick_infos$duration ~ tick_infos$start - sim_start, xlab = 'Time [s]', ylab = 'Duration [s]', t = 'b' )
```
...
bwplot(blocks$total_size / 1000, ylab='Block size [kb]')
```
...
densityplot(~propagation_duration, data=blocks_received, auto.key = list(space = "right"), ref = TRUE, xlab = 'Duration [s]')
```
...
densityplot(~propagation_duration, data=txs_received, auto.key = list(space = "right"), ref = TRUE, xlab = 'Duration [s]')
```
...
bwplot(blocks$txs, ylab='Transactions')
```
