...
ticks <- read.csv2('../ticks.csv', row.names = NULL, header = FALSE)
...
**`r format(utctime(step_times$timestamp[1]), '%Y-%m-%d %H:%M:%S %Z')`**
and ended at
**`r format(utctime(step_times$timestamp[length(step_times)]), '%Y-%m-%d %H:%M:%S %Z')`**
...
```{r events_summar}
planned_blocks <- length(unlist(apply(ticks, c(1,2), function(x) grep("block", x)), use.names = FALSE))
planned_txs    <- length(unlist(apply(ticks, c(1,2), function(x) grep("tx", x)), use.names = FALSE))
df             <- data.frame(
                    c(nrow(ticks), nrow(tick_infos) + 1),
...
xyplot(cpu_time_delta$usage ~ cpu_time_delta$timestamp - sim_start, xlab = 'Time [s]', ylab = 'CPU usage [%]', t = 'l' )
```
...
```{r simulation}
diff_steps              <- data.frame(diff(step_times$timestamp))
colnames(diff_steps)    <- c('timestamp')
