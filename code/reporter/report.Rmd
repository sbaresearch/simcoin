...
```{r block_size_stats, eval=nrow(blocks)>0}
...
```{r blocks_received_stats, eval=nrow(blocks)>0}
...
```{r block_propagation_stats, eval=nrow(blocks)>0}
...
```{r block_std_propagation_stats, eval=nrow(blocks)>0}
...
```{r txs_accepted_stats, eval=nrow(txs)>0}
...
```{r txs_per_block_stats, eval=nrow(blocks)>0}
...
```{r calc_tick_duration, eval=nrow(tick_infos)>0, include=FALSE}
...
```{r tick_duration_stats, eval=nrow(tick_infos)>0}

```
`r tick_infos$tick_duration`

```{r ticks_stats, eval=nrow(tick_infos)>0}
...
```{r mempool_stats, eval=nrow(mempool)>0}
