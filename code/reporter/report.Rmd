...
library(knitr)
opts_chunk$set(fig.path='RmdFigs/', warning=FALSE, message=FALSE)
...
kable(args_transposed, col.names = c('value'), format = 'latex') %>%
...
kable(diff_steps, col.names = c('duration [s]', 'duration [min]'), format = 'latex') %>%
  kable_styling(latex_options = "scale_down")
```
...
kable(unclass(summary(tick_infos$duration)), digits = 3, caption = 'Tick duration [s]', format = 'latex')
```

```{r tick_duration_boxplot, fig.cap='Tick duration'}
...
During the run every 5 seconds the files `/proc/stat` and `/proc/meminfo` were read to monitor the system. In total 100 snapshots were collected. Figure A shows the CPU utilization and figure B shows the memory usage during the run.
...
```{r blocks_distribution, fig.cap='Block creators'}
...
```{r stale, fig.cap='Accepted/stale blocks'}
...
kable(unclass(summary(blocks$total_size / 1000)), digits = 3, caption = 'Block size [kB]', format = 'latex')
```

```{r block_size_boxplot, fig.cap='Block size'}
boxplot(blocks$total_size / 1000, ylab='Block size [kb]')
```

```{r block_propagation_summary}
kable(unclass(summary(blocks_received$propagation_duration * 1000)), digits = 3, caption = 'Block propagation [ms]', format = 'latex')
```

```{r block_propagation_density, fig.cap='Block propagation density'}
...
```{r txs_distribution, fig.cap='Transaction creators'}
...
kable(unclass(summary(txs_received$propagation_duration * 1000)), digits = 3, caption = 'Transactions propagation [ms]', format = 'latex')
```

```{r tx_propagation_density, fig.cap='Transaction propagation density'}
...
```{r txs_per_block_boxplot, fig.cap='Transactions in block'}
...
```{r block_exces_hist, eval=nrow(block_exces)}
hist(block_exces$rel_timestamp - sim_start, xlab = 'Time [s]')
```

```{r block_exces_node_distribution, eval=nrow(block_exces), fig.height=6}
...
```{r block_exces_distribution, eval=nrow(block_exces), fig.height=6}
...
```{r tx_exces_distribution, eval=nrow(tx_exces), fig.height=6}
...
```{r rpc_exces, eval=nrow(rpc_exces)}
rpc_exces$rel_timestamp <- rpc_exces$timestamp - sim_start
hist(rpc_exces$rel_timestamp, xlab = 'Time [s]')
```

```{r rpc_exces_node_distribution, eval=nrow(rpc_exces), fig.height=6}
...
```{r rpc_exces_distribution, eval=nrow(rpc_exces), eval=nrow(rpc_exces), fig.height=6}
