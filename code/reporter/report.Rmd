...
blocks_create         <- read.csv2("blocks_create.csv", dec=".")
blocks_reconstructed  <- read.csv2("blocks_reconstructed.csv", dec=".")
blocks_received       <- read.csv2("blocks_received.csv", dec=".")
block_exces           <- read.csv2("block_exceptions.csv", dec=".")
peer_logic_validation <- read.csv2("peer_logic_validation.csv", dec=".")
update_tip            <- read.csv2("update_tip.csv", dec=".")
...
```{r blocks_setup}
valid_blocks      <- rbind(peer_logic_validation, update_tip[, !(names(update_tip) %in% c('tx', 'height'))])
block_propagation <- rbind(blocks_reconstructed, blocks_received, valid_blocks)
...
valid_blocks <- merge(valid_blocks, consensus_chain, all.x = TRUE)
valid_blocks$stale[is.na(valid_blocks$stale)] <- 'Stale'

stale_block <- t(table(valid_blocks$stale))
...
factor_stale_block <- t(table(factor(valid_blocks$stale, levels = c('Accepted', 'Stale'))))
...
bwplot(tips$branchlen, xlab='Length in blocks')
```
