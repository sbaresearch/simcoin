...
parsed_blocks   <- blocks_create %>% summarise(n = n())

block_count = sum(str_count(analysed_ticks, 'block'))
summary           <- data.frame(block_count, analysed_blocks$blocks, parsed_blocks$n)
...
xyplot(tick_infos$duration ~ tick_infos$actual_start - sim_start,
...
factor_stale_block <- t(table(factor(blocks_create$stale, levels = c('Accepted', 'Stale'))))
stale_rate <- data.frame(c(round(prop.table(factor_stale_block)[, c('Stale')] * 100, 3)))
...
stale_block <- t(table(blocks_create$stale))
...
```{r block_stale_distribution, fig.cap='Stale blocks distribution', fig.pos='H'}
stale_block_nodes  <- blocks_create %>% filter(stale == 'Stale') %>% select(node)
stale_block_groups <- merge(stale_block_nodes, nodes, by.x = 'node', by.y = 'name') %>% select(group)
stale_per_group <- t(table(stale_block_groups$group))
rownames(stale_per_group) <- c('Run')
...
blocks_with_group <- merge(blocks_create, nodes, by.x = 'node', by.y = 'name')
group_blocks      <- blocks_with_group %>%
  group_by(group) %>%
  summarise(block_count = n(), block_share = round(n()/nrow(blocks_create) * 100, 2))
