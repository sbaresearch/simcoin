...
kable(unclass(summary(tick_infos$duration)), digits = 3, caption = 'Tick duration [s]')
```
...
kable(unclass(summary(blocks$total_size / 1000)), digits = 3, caption = 'Block size [kB]')
```

```{r size}
hist(blocks$total_size / 1000, xlab = 'Size [kb]')
```
...
kable(unclass(summary(blocks_received$propagation_duration * 1000)), digits = 3, caption = 'Block propagation [ms]')
```
...
*Note: Some transactions may reached other nodes included in a block. Hence they wouldn't appear in this statistics.*

```{r txs_median_propagation_stats}
kable(unclass(summary(txs_received$propagation_duration * 1000)), digits = 3, caption = 'Transactions propagation [ms]')
```

### Transactions per block
```{r txs_per_block_stats}
kable(unclass(summary(blocks$txs)), digits = 0, caption = 'Transactions per block')
```
