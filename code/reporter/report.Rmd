...
blocks_received_stats$values <- c(
  ...
  min(blocks$total_received),
  max(blocks$total_received)
)
...
hist(blocks$total_received, xlab = 'Nodes')
```

### Propagation time of blocks
`r if(!is.na(table(blocks$total_received)['0'])) {paste('*The', table(blocks$total_received)['0'], 'blocks which were never received by another node are not included in this section*', sep=' ')}`
...
## Transactions
There were `r nrow(txs)` transaction created in this simulation. `r table(txs$total_accepted)['0']` of `r nrow(txs)` never reached another node.
...
txs_accepted_stats <- data.frame(c('Mean txs acception rate', 'Median txs acception rate', 'Std txs acception rate', 'Min txs acception rate', 'Max txs acception rate'))
txs_accepted_stats$values <- c(
  round(mean(txs$total_accepted), 3),
  round(median(txs$total_accepted), 3),
  round(sd(txs$total_accepted), 3),
  min(txs$total_accepted),
  median(txs$total_accepted)
)
...
hist(txs$total_accepted, xlab = 'Nodes')
```

### Transaction acception time
`r if(!is.na(table(txs$total_accepted)['0'])) {paste('*The', table(txs$total_accepted)['0'], 'transaction which never reached another node are not included in this section*', sep=' ')}`
