...
analysed_ticks   <- head(tail(tick_infos, -skip_ticks), -skip_ticks)
write.csv(analysed_ticks, 'analysed_ticks.csv', row.names=FALSE, quote=FALSE)
...
blocks <- blocks %>% filter(timestamp > head(analysed_ticks$start, 1) & timestamp < tail(analysed_ticks$end, 1)) %>%
...
for (i in 1:length(files)) {
  ...
  data <- data %>% filter(timestamp > head(analysed_ticks$start, 1) & timestamp < tail(analysed_ticks$end, 1))
...
for (i in 1:length(files)) {
  data <- read.csv(paste(files[i], '_raw.csv', sep = ''))
  data <- data %>% filter(timestamp > head(analysed_ticks$start, 1))
  write.csv(data, paste(files[i], 'csv', sep = '.'), row.names=FALSE, quote=FALSE)
}
```
...
```{r events_summary}
events <- data.frame(
  c(sum(analysed_ticks$blocks), nrow(blocks_create)),
  c(sum(analysed_ticks$txs), nrow(txs) - nrow(blocks_create))
)
row.names(events)  <- c('Planned', 'Run')
kable(events,
      col.names = c('Blocks', 'Transactions'), caption = 'Events summary', format = 'latex') %>%
