...
# knitr::opts_knit$set(root.dir = '/tmp/knitr')
```
...
mempool_shots <- read.csv2("mempool_snapshots.csv", dec=".")
...
There were `r nrow(tx_exces)` mempool snapshots parsed from log file.

```{r exces, eval=nrow(tx_exces)>0}
hist(tx_exces$timestamp)
```

### Distribution
```{r exces_node_distribution, eval=nrow(tx_exces)>0, fig.height=6}
...
```{r exces_error_message_distribution, eval=nrow(tx_exces)>0, fig.height=6}
...
There were `r nrow(mempool_shots)` mempool snapshots parsed from log file.

Mean: `r round(mean(mempool_shots$txs), 3)` txs

Median: `r round(median(mempool_shots$txs), 3)` txs

Standard deviation: `r round(sd(mempool_shots$txs), 3)` txs

```{r mempool_txs, eval=nrow(mempool_shots)>0}
hist(mempool_shots$txs)
```
