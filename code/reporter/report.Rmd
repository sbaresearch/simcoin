...
```{r blocks_creation_time, fig.cap='Histogram of block creation time'}
hist(blocks$rel_timestamp, xlab = 'Time [s]', main = NULL)
```
...
```{r size, fig.cap='Histogram of block size'}
hist(blocks$total_size / 1000, xlab = 'Size [kb]', main = NULL)
```
...
```{r txs_creation_time, fig.cap='Histogram of transaction creation'}
hist(txs$rel_timestamp, xlab = 'Time [s]', main = NULL)
```
...
```{r txs_per_block, fig.cap='Histogramm of transactions in block'}
hist(blocks$txs, xlab = 'Transactions', main = NULL)
```
