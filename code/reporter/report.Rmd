...
diff_steps_summary              <- rbind(diff_steps, apply(diff_steps, 2, sum))
diff_steps_summary$duration_min <- diff_steps_summary$timestamp/60
diff_steps_summary              <- diff_steps_summary %>%
  mutate_at(.vars = vars(timestamp, duration_min), .funs = funs(round(.,2)))
rownames(diff_steps_summary)   <- c('preparation step', 'simulation step', 'postprocessing step', 'total')

kable(diff_steps_summary, col.names = c('duration [s]', 'duration [min]'), caption = 'Duration of different steps', format = 'latex') %>%
  kable_styling(latex_options = c('scale_down','HOLD_position'))
```

```{r step_times_barchart, fig.cap='Step times', fig.pos='H', fig.height=3}
diff_steps             <- data.frame(diff(step_times$timestamp))
t_diff_steps           <- t(diff_steps)
colnames(t_diff_steps) <- c('preparation', 'simulation', 'postprocessing')
rownames(t_diff_steps) <- c('run')
barchart(t_diff_steps, stack = TRUE, ref = FALSE, xlab = 'Time [s]', auto.key = list(space='top',text=colnames(t_diff_steps)))
```
