...
`r #TODO merge general_infos.json`
...
```{r cpu_time_plot, fig.cap='CPU usage over time', fig.env='figure*'}
...
cpu_time_delta         <- merge(cpu_time_delta, simulation_starts)
cpu_time_delta$elapsed <- cpu_time_delta$timestamp - cpu_time_delta$run_start
xyplot(cpu_time_delta$usage ~ cpu_time_delta$elapsed, groups = cpu_time_delta$tag, xlab = 'Time [s]', ylab = 'CPU usage [%]', t = 'l' )
```

```{r memory_xyplot, fig.cap='Memory usage over time', fig.env='figure*'}
...
xyplot(memory$usage ~ memory$elapsed, groups = memory$tag, xlab = 'Time [s]', ylab = 'Memory usage [%]', t = 'l' )
```


```{r step_times_barchart, fig.cap='Step times', fig.env='figure*', fig.height=length(unique(step_times$tag)) + 1}
diff_steps <- step_times %>%
  group_by(tag) %>%
  mutate(diff = c(NA, diff(timestamp))) %>%
  filter(!is.na(diff))
diff_steps <- dcast(diff_steps, tag~type, value.var = 'diff')
rownames(diff_steps) <- diff_steps[,1]
diff_steps           <- diff_steps[,-1]
diff_steps           <- diff_steps[, c(3,2,1)]
colnames(diff_steps) <- c('preparation', 'simulation', 'postprocessing')
barchart(as.matrix(diff_steps), stack = TRUE, ref = FALSE, xlab = 'Time [s]', auto.key = list(space='top',text=colnames(diff_steps)))
```
...
xyplot(tick_infos$duration ~ ticks$elapsed, groups = ticks$tag, xlab = 'Time [s]', ylab = 'Duration [s]', t = 'l' )
```
...
```{r stale, fig.cap='Stale blocks', fig.env='figure*', fig.height=length(unique(blocks$tag)) + 1}
stale_block <- t(table(blocks$stale, blocks$tag))
barchart(stale_block, stack = TRUE, ref = FALSE, xlab = 'Blocks', auto.key = list(space='top',text=colnames(stale_block)))
```
...
densityplot(~propagation_duration, data=blocks_received, groups = tag, auto.key = list(space = "right"), ref = TRUE, xlab = 'Duration [s]', plot.points = FALSE)
```
...
densityplot(~propagation_duration, data=txs_received, groups = tag, auto.key = list(space = "right"), ref = TRUE, xlab = 'Duration [s]', plot.points = FALSE)
```
