...
df             <- data.frame(
                    ...
                    c(planned_blocks, data.frame(table(blocks_create$tag))[,2]),
                    c(planned_txs, data.frame(table(txs$tag))[,2] - data.frame(table(blocks_create$tag))[,2])
                  )
...
bwplot(tips$branchlen ~ tips$tag, ylab = 'Length in blocks')
```
