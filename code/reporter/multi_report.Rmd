...
general_infos_table <- data.frame(
                    c(format(utctime(step_times$timestamp[1]), '%Y-%m-%d %H:%M:%S %Z'),
                      ...
                      length(ticks) - length(analysed_tick_infos))
                  )
row.names(general_infos_table)  <- c('Start', 'End', 'Commit', 'Planned ticks', 'Planned analysed ticks', 'Skipped ticks')
...
```{r memory_xyplot, fig.cap='Memory usage over time', fig.pos='H', fig.height=3}
...
```{r block_stale_barchart, fig.cap='Stale blocks', fig.pos='H'}
...
```{r block_propagation_boxplot, fig.cap='Block propagation boxplot', fig.pos='H'}
...
```{r tx_propagation_boxplot, fig.cap='Transaction propagation boxplot', fig.pos='H'}
...
bwplot(blocks_create$txs ~ blocks_create$tag, ylab = 'Transactions')
```
