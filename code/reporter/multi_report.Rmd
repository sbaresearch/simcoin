...
analysed_ticks <- read.csv('analysed_ticks.csv', dec='.')
...
analysed_blocks  <- analysed_ticks %>% group_by(tag) %>% summarise(blocks = sum(blocks))
...
```{r tx_events_summary}
analysed_txs     <- analysed_ticks %>% group_by(tag) %>% summarise(txs = sum(txs))
