...
class Runner:
    ...
    def run(self):
        try:
            self.context.general_infos['preparation_start'] = time.time()
            self.prepare.execute()

            self.context.general_infos['simulation_start'] = time.time()
            logging.info(config.log_line_sim_start)
            self.event.execute()
            logging.info(config.log_line_sim_end)

            self.context.general_infos['postprocessing_start'] = time.time()
